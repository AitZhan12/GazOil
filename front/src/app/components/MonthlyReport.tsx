  ...
  const operatorStats = useMemo(() => {
    const stats: MonthlyOperatorStats[] = operators
      ...
      .map(operator => {
        ...
        const voucherDebt = operatorShifts
          .reduce((sum, shift) => {
            const debt = shift.cashCollected
  ...
  const totals = useMemo(() => {
    return operatorStats.reduce(
      (acc, stat) => ({
        ...
        voucherDebt: acc.voucherDebt + stat.voucherDebt,
      }),
      {
        shiftsCount: 0,
        totalLiters: 0,
  ...
  const handleExport = () => {
    ...
    if (selectedOperator !== ALL) {
      ...
      rows.push([
        ...
        (selectedStat?.voucherDebt ?? 0).toFixed(2),
      ]);
    ...
    const rowsHtml = isSingle
      ...
      : operatorStats.map(stat => `
          <tr>
            ...
            <td class="num">${formatLiters(stat.voucherDebt)}</td>
          </tr>`).join('');
    ...
    printPdf(`${title} ${selectedMonthLabel}`, `
      ...
      <table>
        <tr>
          ...
          ${!isSingle ? '<th class="num">Долг по кассе</th><th class="num">Долг талонов</th>' : ''}
        </tr>
        ${rowsHtml}
        <tr class="total">
          <td>ИТОГО</td>
          ...
          ${!isSingle ? `<td class="num">${formatCurrency(total?.cashDebt ?? 0)}</td><td class="num">${formatLiters(total?.voucherDebt ?? 0)}</td>` : ''}
        </tr>
      </table>
    `);
  };
    ...
    <div className="space-y-4">
      ...
      {isSingle && hasData && selectedStat && (
          ...
          <SummaryCard label="ЗП база" value={formatCurrency(selectedStat.baseSalary)} />
            ...
            <SummaryCard label="Долг талонов" value={formatLiters(selectedStat.voucherDebt)} />
        </div>
      )}
            ...
            <table className="w-full border-collapse min-w-[1020px]">
              <thead>
                <tr className="bg-[#f8fafc] border-b border-[#d1d9e6]">
                  ...
                  <th className={`${thCell} text-right bg-red-50`} style={thStyle}>Долг талонов</th>
                </tr>
              </thead>
              <tbody>
                {operatorStats.map((stat, idx) => (
                  ...
                  >
                    ...
                    <td className={`px-4 py-2.5 text-right font-mono ${stat.voucherDebt > 0 ? 'text-red-700 bg-red-50/60 font-semibold' : 'text-slate-500 bg-red-50/40'}`} style={{ fontSize: '13px' }}>{formatLiters(stat.voucherDebt)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-[#f0f2f5] border-t-2 border-[#d1d9e6]">
                  ...
                  <td className="px-4 py-2.5 text-right font-mono text-red-800 bg-red-100/60" style={{ fontSize: '14px', fontWeight: 700 }}>{formatLiters(totals.voucherDebt)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
      ...
      <div className="flex items-start gap-2 px-4 py-3 bg-amber-50 border border-amber-200 rounded-lg" style={{ fontSize: '12px' }}>
        ...
        <span className="text-amber-800">
          ...
          «Настройки». Это сумма до удержаний (ИПН/ОПВ/ВОСМС не считаются). Долг по кассе и
          талонам показан справочно только за выбранный месяц и автоматически из зарплаты не списывается.
